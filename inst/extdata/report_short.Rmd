...
measure_fail$SNR <- -1
measure_fail$SFNR <- -1
measure_fail$percentFluc <- 1
measure_fail$drift <- 1
measure_fail$driftfit <- 1
measure_fail$rdc <- -1
...
oneyeardat[[p$name]] <- subset(longreport[[p$name]], scandate_epoch > as.numeric(Sys.Date())-365)
  ...
  for(m in these_measures){
    ...
    #fail if greater than 2 sd, AND in the wrong direction. Otherwise pass. Separate direction check, because if vv == mm then it's NaN.
    dirfail <- (vv-mm)/abs(vv-mm) == measure_fail[[m]]
    if (is.na(dirfail)){dirfail <- FALSE}
    eachpass[index] <- !(abs((vv-mm)/ss) > 2 & dirfail)
  }
  if(all(eachpass)){
    passcolor[[p$name]] = text_green
    passtext[[p$name]] = 'All metrics in acceptable range.'
...
## 60 day trend  
Standardized QA metrics from past 60 days. Outlined dots are from the most recent QA scan (Green = within 2SD, Red = outside 2SD and poor performance, Blue = outside 2SD and acceptable performance). Black/Grey dots are scans within the past 60 days; Lighter grey = older scans. Shaded regions represent 1 and 2 SDs, based on QA data from the past 365 days. For Drift, Drift-Fit, and Percent Fluctuation, *lower is better*. For RDC, SFNR, and SNR, *higher is better*.
...
Historical QA metrics from scans using the bullet phantom (orange) and the fBIRN phantom (blue) since 2022. The orange and blue lines represent 60-day smoothing. Orange and light-blue dots are individual QA scans. Other colors indicate room lighting conditions: Black = all lights off. Pink = all lights on. Green = Front lights on + Back lights off. Dark Blue = Back lights on + Front lights off.

Red vertical lines indicate dates of gradient changes:
